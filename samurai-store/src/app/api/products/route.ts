...
try {
    ...
    // SQL文に埋め込むパラメータを構築
    const productsParams: (string | number | null)[] = [...whereParams, perPage, offset];
    const countParams: (string | number | null)[] = [...whereParams];

    // 2つのデータベース操作を並行処理で実施
    const [products, totalItemsResult] = await Promise.all([
  // ✅ Product[] → Product に修正、パラメータも productsParams に修正
  executeQuery<Product>(`
    ...
    LIMIT ?
    OFFSET ?
    ;`, productsParams  // ← [perPage, offset] から productsParams に変更済みか確認
  ),
