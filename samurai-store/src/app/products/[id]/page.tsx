...
import { AUTH_TOKEN } from '@/lib/auth'; // AUTH_TOKENの定義元に合わせてください
...
// 商品IDに紐づくお気に入り状態を取得
async function getIsFavorited(id: string): Promise<boolean> {
  try {
...
export default async function ProductDetailPage(props: ProductDetailPageProps) {
 ...
 // ログイン済みの場合のみお気に入り状態も並行取得
const loggedIn = await isLoggedIn();

const [product, reviewsResponse, isFavorited] = await Promise.all([
  getProduct(productId),
  getReviews(productId),
  loggedIn ? getIsFavorited(productId) : Promise.resolve(false),
]);
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-1/2 space-y-6 pt-4">
          ...
          <div className="space-y-6 mt-8">
            ...
            {loggedIn && (
  <FavoriteControls productId={product.id} initialIsFavorited={isFavorited} />
)}
          </div>
        </div>
      </div>
